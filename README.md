...
- **PDF ingestion and chunking** — via `langchain` document loaders + text splitters
- **Local embeddings** — HuggingFace `sentence-transformers/all-MiniLM-L6-v2`
- **FAISS vector store** — semantic retrieval, cached to disk after first build
- **Two hallucination guardrails**
  1. **Distance threshold** — rejects questions with no relevant match in the document
  2. **LLM-as-judge coverage check** — a second LLM call verifies retrieved context actually contains enough info before generating an answer
...
| 5 | `ask_question` | Retrieves relevant chunks, runs guardrails, then generates a grounded answer |

```
PDF → Load → Split into chunks → Embed → FAISS index
                                              │
User question ──► Retrieve top-k chunks ──► Guardrails ──► LLM answer
                                              │
...
1. Copy `.env.example` to `.env` and add your `GROQ_API_KEY` (get one at [console.groq.com](https://console.groq.com))
2. Place your source PDF in the project root as `python_handbook.pdf`, or set `PDF_PATH` in `.env` to point elsewhere
3. Run the app:
...
> On first run, the app will build and cache a FAISS index (`faiss_index/`) from the PDF. Subsequent runs load the cached index and start faster.
